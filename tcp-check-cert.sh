...
sh_ver="1.3.2"
github="raw.githubusercontent.com/ss-daily-backup/Linux-NetSpeed-chiakge/master"
...
installbbr(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N http://${github}/bbr/debian-ubuntu/linux-headers-${kernel_version}-all.deb
		wget -N http://${github}/bbr/debian-ubuntu/${bit}/linux-headers-${kernel_version}.deb
		wget -N http://${github}/bbr/debian-ubuntu/${bit}/linux-image-${kernel_version}.deb
...
installbbrplus(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N http://${github}/bbrplus/debian-ubuntu/${bit}/linux-headers-${kernel_version}.deb
		wget -N http://${github}/bbrplus/debian-ubuntu/${bit}/linux-image-${kernel_version}.deb
		dpkg -i linux-headers-${kernel_version}.deb
		dpkg -i linux-image-${kernel_version}.deb
		cd .. && rm -rf bbrplus
	fi
...
installlot(){
	if [[ "${release}" == "centos" ]]; then
		...
		bash <(wget -qO- "http://${github}/Debian_Kernel.sh")
...
startbbrmod(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N http://${github}/bbr/tcp_tsunami.c
...
Update_Shell(){
	...
	sh_new_ver=$(wget -qO- "http://${github}/tcp.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -N http://${github}/tcp.sh && chmod +x tcp.sh
